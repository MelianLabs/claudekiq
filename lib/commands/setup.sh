...
_generate_cq_md() {
...

  {
    ...
    # Available workflows with step summaries
    ...
    if [[ -d "$workflows_dir" ]]; then
      ...
      for wf in "$workflows_dir"/*.yml "$workflows_dir"/*.yaml; do
        ...
        if [[ -n "$wf_json" ]]; then
          local step_summary
          step_summary=$(echo "$wf_json" | jq -r '
            [.steps // [] | .[] |
              .id + (if .gate == "human" then " (human)" elif .gate == "review" then " (review)" else "" end) +
              (if .type == "agent" then " [agent]" elif .type == "parallel" then " [parallel]" elif .type == "workflow" then " [sub-workflow]" else "" end)
            ] | join(" → ")
          ' 2>/dev/null || true)
          [[ -n "$step_summary" ]] && echo "  Steps: ${step_summary}"
          # Params if any
          local params_list
          params_list=$(echo "$wf_json" | jq -r '
            .params // {} | to_entries | map(.key + if .value.required == true then " (required)" else "" end) | join(", ")
          ' 2>/dev/null || true)
          [[ -n "$params_list" ]] && echo "  Params: ${params_list}"
        fi
      done
    fi
    ...
    # Usage patterns
    echo '## Usage Patterns'
    echo '- Start a workflow: `/cq <name>` or `cq start <name> --key=val`'
    echo '- Interactive picker: `/cq` (lists workflows, lets you choose)'
    echo '- Monitor progress: `/cq status` or `cq status <run_id> --json`'
    echo '- Approve a gate: respond to the prompt when asked'
    echo '- Skip a step: `cq skip <run_id>`'
    echo '- Cancel: `cq cancel <run_id>`'
    echo ''

    # Team workflow notes (if multiple agents or custom concurrency)
    if [[ -f "$settings_file" ]]; then
      local agent_count concurrency
      agent_count=$(jq '.agents // [] | length' "$settings_file" 2>/dev/null || echo "0")
      concurrency=$(jq -r '.concurrency // 1' "$settings_file" 2>/dev/null || echo "1")
      if [[ "$agent_count" -gt 1 || "$concurrency" -gt 1 ]]; then
        echo '## Team Workflow'
        echo 'Each workflow run is independent — multiple runs can execute concurrently.'
        echo "Configure max concurrency: \`cq config set concurrency N\` (current: ${concurrency})"
        echo ''
      fi

      # Notifications if configured
      local has_notifications
      has_notifications=$(jq '.notifications // {} | to_entries | map(select(.value != null)) | length' "$settings_file" 2>/dev/null || echo "0")
      if [[ "$has_notifications" -gt 0 ]]; then
        echo '## Notifications'
        jq -r '.notifications // {} | to_entries[] | select(.value != null) | "- " + .key + ": `" + .value + "`"' "$settings_file" 2>/dev/null || true
        echo ''
      fi
    fi
